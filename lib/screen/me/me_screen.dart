...
class MeScreen extends StatelessWidget {
          ...
          return Column(children: [
              ...
              child: Row(children: [
                  ...
                  CircleAvatar(
                    ...
                    backgroundImage: (avatar != null && avatar.isNotEmpty)
                        ? NetworkImage(avatar.startsWith('http')
                            ? avatar : 'http://10.0.2.2:8000$avatar') as ImageProvider
                        : null,
                  ...
                  if (auth.isVip)
                    Positioned(bottom: 0, right: 0,
                      child: Container(
                        ...
                        child: const Icon(Icons.diamond, color: Colors.white, size: 10),
                      )),
                ]),
                  ...
                  if (isLoggedIn) ...[
                    ...
                    Row(children: [
                      ...
                      Container(
                        ...
                        child: Text('Lv.${auth.readingLevel}',
                            style: TextStyle(color: depperBlue, fontSize: 11)),
                      ),
                    ]),
                  ],
                ])),
                if (!isLoggedIn)
                  ElevatedButton(
                    ...
                    child: const Text('Sign In', style: TextStyle(color: Colors.white, fontSize: 13)),
                  ),
              ]),
            ),
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [depperBlue.withOpacity(0.9), const Color(0xFF003d80)],
                    ),
                  ...
                  child: Row(children: [
                    ...
                    const Spacer(),
                    const Text('Buy More', style: TextStyle(color: Colors.white70, fontSize: 12)),
            ...
            Expanded(
              child: ListView(padding: const EdgeInsets.only(bottom: 20), children: [
                _section([
                  _item(Icons.timelapse_rounded, 'History', Colors.lightBlueAccent,
                      () => Get.to(() => const LibraryScreen(),
                          arguments: {'value': 1, 'isProfile': true})),
                  if (isLoggedIn)
                    _item(Icons.monetization_on, 'Coin Store', Colors.orange,
                        () => Get.to(() => const CoinStoreScreen())),
                  _item(Icons.download, 'Downloads', Colors.blue,
                      () => Get.to(() => DownloadScreen())),
                  _item(Icons.dark_mode, 'Display Mode', Colors.purple,
                      () => _showThemeSheet(context)),
                  _item(Icons.favorite_outline, 'Reading Preferences', Colors.pink, () {}),
                ]),
                const SizedBox(height: 10),
                _section([
                  Obx(() => _item(Icons.notifications_outlined, 'Notifications', Colors.orange,
                    () {
                      Get.put(NotificationController());
                      Get.to(() => const NotificationScreen());
                    },
                    badge: ctrl.unreadCount.value > 0 ? '${ctrl.unreadCount.value}' : null,
                  )),
                ]),
                const SizedBox(height: 10),
                _section([
                  if (isLoggedIn && auth.isAuthor)
                    _item(Icons.dashboard_outlined, 'Author Dashboard', Colors.teal,
                        () => Get.to(() => const AuthorDashboardScreen()))
                  else if (isLoggedIn)
                    _item(Icons.edit_outlined, 'Become an Author', Colors.lightBlue,
                        () => _becomeAuthorDialog(ctrl))
                  else
                    _item(Icons.login, 'Sign In to Write', Colors.lightBlue,
                        () => Get.to(() => const LoginScreen())),
                  _item(Icons.info_outline, 'About NoveluX', Colors.amber,
                      () => Get.to(() => AboutScreen())),
                  _item(Icons.help_outline, 'Help & Feedback', Colors.orange, () {}),
                  if (isLoggedIn)
                    _item(Icons.logout, 'Sign Out', Colors.red,
                        () => _signOutDialog(auth)),
                  if (!isLoggedIn)
                    _item(Icons.person_add_outlined, 'Create Account', Colors.green,
  ...
  Widget _item(IconData icon, String label, Color color, VoidCallback onTap, {String? badge}) =>
    ListTile(
      leading: Container(
  ...
  void _showThemeSheet(BuildContext ctx) {
    showModalBottomSheet(
      backgroundColor: Colors.transparent, context: ctx,
      builder: (_) => Padding(
        padding: const EdgeInsets.all(8),
        child: Container(
          height: 180,
          ...
          child: Column(children: [
            ...
            const Text('Display Mode', style: TextStyle(color: Colors.white, fontSize: 15)),
            const Divider(color: Colors.black26),
            ...['System Default', 'Dark', 'Light'].map((mode) => ListTile(
              leading: Radio<String>(
                value: mode, groupValue: 'Dark', onChanged: (_) {},
  ...
  void _becomeAuthorDialog(MeController ctrl) {
    Get.dialog(AlertDialog(
      ...
      title: const Text('Become an Author', style: TextStyle(color: Colors.white)),
      content: const Text('Start publishing stories and earn coins on NoveluX!',
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          onPressed: () { Get.back(); ctrl.becomeAuthor(); },
          child: const Text('Become Author', style: TextStyle(color: Colors.white)),
        ),
      ],
    ));
  }
