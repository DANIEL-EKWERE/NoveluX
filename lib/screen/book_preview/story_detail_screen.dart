...
class StoryDetailController extends GetxController {
  ...
  void loadChapters(String slug) async {
    ...
    if (res['success']) {
      chapters.value = res['data'] is List
          ? res['data']
          : (res['data']['results'] ?? []);
    }
  }
...
class StoryDetailScreen extends StatelessWidget {
              ...
              actions: [
                Obx(() => IconButton(
                      icon: Icon(
                        ...
                        color: ctrl.isBookmarked.value ? depperBlue : Colors.white,
                      ),
                ...
                PopupMenuButton<int>(
                  ...
                  itemBuilder: (context) => [
                    const PopupMenuItem(value: 0, child: Text('Share', style: TextStyle(color: Colors.white))),
                    const PopupMenuItem(value: 1, child: Text('Report', style: TextStyle(color: Colors.white))),
                  ],
                ),
              ],
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      Text(author['username'] ?? 'Unknown Author',
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          _stat('${story['total_views'] ?? 0}', 'Views', Icons.visibility_outlined),
                          _divider(),
                          _stat('${story['total_chapters'] ?? 0}', 'Chapters', Icons.menu_book_outlined),
                          _divider(),
                          _stat('${double.tryParse(story['average_rating'].toString())?.toStringAsFixed(1) ?? '0.0'}', 'Rating', Icons.star_outline),
                        ],
                      ),
                    ),
                    const SizedBox(height: 16),
                            ...
                            onPressed: () {
                              if (ctrl.chapters.isNotEmpty) {
                                ...
                                Navigator.push(context, CupertinoPageRoute(
                                  builder: (_) => NovelUpReadingInterface(
                                    storySlug: slug,
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      ...
                      Obx(() => Text('${ctrl.chapters.length} chapters',
                  ...
                  (context, index) {
                    ...
                    return ListTile(
                      ...
                      leading: Container(
                        ...
                        child: Center(
                          child: Text(
                            '${ch['chapter_number']}',
                            style: TextStyle(
                              color: isUnlocked || !isLocked ? depperBlue : Colors.grey,
                              fontWeight: FontWeight.bold,
                              fontSize: 12,
                            ),
                          ),
                      ...
                      title: Text(
                        ...
                        style: TextStyle(
                          color: isUnlocked || !isLocked ? Colors.white : Colors.grey,
                          fontSize: 13,
                        ),
                        maxLines: 1,
                      ...
                      subtitle: Row(children: [
                        ...
                        if (isLocked && !isUnlocked)
                          Row(children: [
                            ...
                            Text('${ch['coin_cost']} coins',
                                style: const TextStyle(color: Colors.orange, fontSize: 11)),
                          ]),
                      ]),
                      trailing: Icon(
                        isLocked && !isUnlocked ? Icons.lock_outline : Icons.play_circle_outline,
                        color: isLocked && !isUnlocked ? Colors.grey[600] : depperBlue,
                        size: 20,
                      ),
  ...
  void _handleChapterTap(BuildContext context, Map ch, String slug, AuthController auth) async {
    final isLocked   = ch['is_locked'] ?? false;
    final isUnlocked = ch['is_unlocked'] ?? false;
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Unlock Chapter', style: TextStyle(color: Colors.white)),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Obx(() => Text(
              ...
              style: TextStyle(
                color: auth.coins >= coinCost ? Colors.green : Colors.redAccent,
                fontSize: 13,
              ))),
        ]),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Get.back();
              final res = await ApiService.unlockChapter(slug, ch['chapter_number']);
              if (res['success']) {
                await auth.fetchMe();
                Navigator.push(context, CupertinoPageRoute(
                  builder: (_) => NovelUpReadingInterface(
                ...
                Get.snackbar('Error', res['error'] ?? 'Failed to unlock',
                    backgroundColor: Colors.red, colorText: Colors.white);
              }
            },
            child: Text('Unlock ($coinCost coins)', style: const TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  Widget _divider() => Container(width: 0.5, height: 40, color: Colors.grey[800]);
}
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GestureDetector(
        ...
        child: Text(
          ...
          style: TextStyle(color: depperBlue, fontWeight: FontWeight.bold, fontSize: 12),
        ),
      ),
    ]);
  }
